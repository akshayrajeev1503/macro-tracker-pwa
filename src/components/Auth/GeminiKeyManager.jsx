...
export default function GeminiKeyManager({ required }) {
  const [storedKey, setStoredKey] = useLocalStorage('GEMINI_API_KEY', '');
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 max-w-md mx-auto border border-gray-200">
      <h3 className="text-lg font-semibold mb-4">
        Gemini API Key {required && <span className="text-red-500">*</span>}
      </h3>
      {isEditing ? (
        <div className="relative mb-4">
          <input
            ...
            placeholder={required ? 'Enter Gemini API Key' : 'Optional'}
